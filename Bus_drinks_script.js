...
function updateField() {
    ...
    if (counting > 0 && counting < 11){ // Since I want a max of 10 guests, this makes sure that the code only works when the entered number is above 0 but below 11. 
        ...
        for (let i = 0; i<counting; i++){ //runs the following code up to the number specified by user.
            ...
            <div id="YesNoBus${i+1}">
                <legend>Would you like to take the bus?</legend>
            ...
            const BusYes = myInp.querySelector(`#yes${i+1}`); // retrieves the yes input
            const BusNo = myInp.querySelector(`#no${i+1}`); // retrieves the no input
...
/*
            ...
            if (oldGuestInfo[i]) { //if there is anything stored in oldGuestInfo[i], proceed with following code
                ...
                if (old.bus === "yes") {
                    ...
                    BusNo.checked = true;
                    GuestiInfo.bus = "no";// sete the current no value to no for validation purposes. 
                }
...
/*
// creates the drink input fields for each guest based on the first question
function updateBus() {
